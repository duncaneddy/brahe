...
/// let result = rkf78.step(0.0, state, None, Some(0.5)).unwrap();
/// ```
...
impl<const S: usize, const P: usize> RKF78SIntegrator<S, P> {
    ...
    fn step_internal(
        ...
        Err(BraheError::NumericalError(format!(
            "RKF78S exceeded maximum step attempts ({}) at t={}",
...
impl RKF78DIntegrator {
    ...
    fn step_internal(
        ...
        Err(BraheError::NumericalError(format!(
            "RKF78D exceeded maximum step attempts ({}) at t={}",
            self.config.max_step_attempts, t
        )))
    }
}
...
mod tests {
    ...
    fn test_rkf78s_integrator_parabola() {
        ...
        while t < 1.0 {
            ...
            let result = rkf78.step(t, state, None, Some(dt)).unwrap();
            state = result.state;
            t += result.dt_used;
        }
    ...
    fn test_rkf78s_integrator_orbit() {
        ...
        while epc < epcf {
            ...
            let result = rkf78.step(epc - epc0, state, None, Some(dt)).unwrap();
            state = result.state;
            epc += result.dt_used;
        }
    ...
    fn test_rkf78s_step_size_increases() {
        ...
        let result = rkf78.step(0.0, state, None, Some(dt_initial)).unwrap();
    ...
    fn test_rkf78s_config_parameters() {
        ...
        let state = SVector::<f64, 1>::new(0.0);
        let result = rkf78.step(0.0, state, None, Some(0.01)).unwrap();
    ...
    fn test_rkf78s_no_limits() {
        ...
        let state = SVector::<f64, 1>::new(0.0);
        let result = rkf78.step(0.0, state, None, Some(0.001)).unwrap();
    ...
    fn test_rkf78s_stm_accuracy() {
        ...
        for i in 0..6 {
            ...
            let result_pert = rkf78.step(0.0, state0_pert, None, Some(dt)).unwrap();
    ...
    fn test_rkf78s_stm_vs_direct_perturbation() {
        ...
        for step in 0..num_steps {
            ...
            let result_pert = rkf78.step(t, state_pert, None, Some(dt)).unwrap();
    ...
    fn test_rkf78d_stm_vs_direct_perturbation() {
        ...
        for step in 0..num_steps {
            ...
            // Propagate perturbed state directly
            let result_pert = rkf78.step(t, state_pert.clone(), None, Some(dt)).unwrap();
    ...
    fn test_rkf78_s_vs_d_consistency() {
        ...
        let result_s = rkf78_s.step(0.0, state_s, None, Some(dt)).unwrap();
        let result_d = rkf78_d.step(0.0, state_d, None, Some(dt)).unwrap();
    ...
    fn test_rkf78s_backward_integration() {
        ...
        let mut t = 0.0;
        while t < 100.0 {
            let result = rkf78.step(t, state_fwd, None, Some(dt_forward)).unwrap();
        ...
        while t > 0.0 {
            ...
            let result = rkf78.step(t, state_back, None, Some(dt_back)).unwrap();
    ...
    fn test_rkf78d_varmat_sensmat() {
        ...
        let result_sensmat = rkf78_sensmat
        ...
        let result_pert = rkf78_pert.step(0.0, state0_pert, None, Some(dt)).unwrap();
    ...
    fn test_rkf78s_varmat_sensmat() {
        ...
        while t_pert < 1.0 {
            ...
            let result = rkf78_pert.step(t_pert, state_pert, None, Some(dt)).unwrap();
    ...
    fn test_rkf78s_params_affect_step_output() {
        ...
        let result_slow = rkf78.step(t, x0, Some(&params_slow), Some(dt)).unwrap();

        // Step with k=5.0 (faster decay)
        let params_fast = SVector::<f64, 1>::new(5.0);
        let result_fast = rkf78.step(t, x0, Some(&params_fast), Some(dt)).unwrap();
    ...
    // =============================================================================
    // Error Tests - dt Contract and Missing sensmat Provider
    // =============================================================================
    ...
    fn test_rkf78s_step_methods_require_dt() {
        ...
        let integrator: RKF78SIntegrator<1, 0> =
            RKF78SIntegrator::new(Box::new(simple_decay_s), None, None, None);
        let state = SVector::<f64, 1>::new(1.0);
    ...
    fn test_rkf78s_control_input_applied() {
        ...
        let f = |_t: f64,
         ...
         -> Result<SVector<f64, 1>, BraheError> { Ok(SVector::<f64, 1>::zeros()) };
        let control =
            |_t: f64,
             _x: &SVector<f64, 1>,
             _p: Option<&SVector<f64, 0>>|
             -> Result<SVector<f64, 1>, BraheError> { Ok(SVector::<f64, 1>::new(1.0)) };
        let integrator: RKF78SIntegrator<1, 0> =
            RKF78SIntegrator::new(Box::new(f), None, None, Some(Box::new(control)));
        let state = SVector::<f64, 1>::new(0.0);
        let result = integrator.step(0.0, state, None, Some(0.1)).unwrap();
    ...
    fn test_rkf78d_control_input_applied() {
        let f = |_t: f64,
                 x: &DVector<f64>,
                 _p: Option<&DVector<f64>>|
         -> Result<DVector<f64>, BraheError> { Ok(DVector::zeros(x.len())) };
        let control =
            |_t: f64,
             _x: &DVector<f64>,
             _p: Option<&DVector<f64>>|
             -> Result<DVector<f64>, BraheError> { Ok(DVector::from_vec(vec![1.0])) };
        let integrator = RKF78DIntegrator::new(1, Box::new(f), None, None, Some(Box::new(control)));
        let state = DVector::from_vec(vec![0.0]);
        let result = integrator.step(0.0, state, None, Some(0.1)).unwrap();
        assert!(result.state[0] > 0.0);
    }
    ...
    fn test_rkf78s_step_with_sensmat_requires_sensmat() {
        ...
        let integrator: RKF78SIntegrator<1, 0> = RKF78SIntegrator::new(

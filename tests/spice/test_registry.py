...
def test_spk_position_in_kernel_shape():
    epc = bh.Epoch.from_date(2025, 1, 1, bh.TimeSystem.UTC)
...
def test_spk_velocity_in_kernel_shape():
    epc = bh.Epoch.from_date(2025, 1, 1, bh.TimeSystem.UTC)
...
def test_spk_state_in_kernel_shape():
    epc = bh.Epoch.from_date(2025, 1, 1, bh.TimeSystem.UTC)
    x = bh.spk_state_in_kernel("de440s", bh.NAIF_SUN, bh.NAIF_EARTH, epc)
    assert x.shape == (6,)


def test_spk_position_in_kernel_agrees_with_pooled():
    ...
    r_in_kernel = bh.spk_position_in_kernel("de440s", bh.NAIF_MOON, bh.NAIF_EARTH, epc)
    np.testing.assert_allclose(r_in_kernel, r_pooled, atol=0.0)


def test_spk_velocity_in_kernel_agrees_with_pooled():
    epc = bh.Epoch.from_date(2025, 1, 1, bh.TimeSystem.UTC)
    v_pooled = bh.spk_velocity(bh.NAIF_MOON, bh.NAIF_EARTH, epc)
    v_in_kernel = bh.spk_velocity_in_kernel("de440s", bh.NAIF_MOON, bh.NAIF_EARTH, epc)
    np.testing.assert_allclose(v_in_kernel, v_pooled, atol=0.0)


def test_spk_state_in_kernel_agrees_with_pooled():
    epc = bh.Epoch.from_date(2025, 1, 1, bh.TimeSystem.UTC)
    x_pooled = bh.spk_state(bh.NAIF_SUN, bh.NAIF_EARTH, epc)
    x_in_kernel = bh.spk_state_in_kernel("de440s", bh.NAIF_SUN, bh.NAIF_EARTH, epc)
    np.testing.assert_allclose(x_in_kernel, x_pooled, atol=0.0)


def test_spk_position_in_kernel_auto_loads():
    """The _in_kernel query auto-loads the named kernel if not already

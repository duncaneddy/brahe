...
EMR, SER, and GSE are named instances of a generic two-body synodic frame, `ReferenceFrame.Synodic(origin, primary, secondary)`. `origin` is a `SynodicOrigin` (`Primary`, `Secondary`, or `Barycenter`); `primary` and `secondary` are the NAIF IDs of the two bodies. For `Primary` and `Secondary` origins, any NAIF ID with SPK coverage is allowed (moons, asteroids, or self-assigned negative IDs); for `Barycenter` origin, both IDs are required to be in $0 \le \text{id} \le 999$ (see below). The axis construction is identical to the formula above, with $\boldsymbol{r}_{12}$ and $\boldsymbol{v}_{12}$ taken between `primary` and `secondary`.

...
/// use brahe::integrators::{RKF45Integrator, FixedStepIntegrator, AdaptiveStepIntegrator, IntegratorConfig};
///
...
impl<const S: usize> AdaptiveStepIntegrator<S> for RKF45Integrator<S> {
    fn step(
        &self,
        t: f64,
        state: SVector<f64, S>,
...
mod tests {
    ...
    fn test_rkf45_integrator_orbit() {
        ...
        let config = IntegratorConfig::adaptive(1e-8, 1e-6);
        let rkf45 = RKF45Integrator::with_config(Box::new(point_earth), None, config);
    ...
    fn test_rkf45_stm_accuracy() {
        ...
        let rkf45 =
            RKF45Integrator::with_config(Box::new(point_earth), Some(Box::new(varmat)), config);
    ...
    #[test]
    fn test_rkf45_stm_vs_direct_perturbation() {
        setup_global_test_eop();

...
Which kernel's precomputed tables a `GravityModel` builds is controlled by `GravityTables` at load time: `GravityTables.Clenshaw` (default), `GravityTables.Cunningham`, or `GravityTables.Both`. Evaluating with a kernel requires its table set to be present; `GravityModel.precompute_clenshaw_tables()` / `precompute_cunningham_tables()` add a table set to an already-loaded model.

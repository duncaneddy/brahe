...
- Unified `ReferenceFrame` type spanning celestial (`CelestialFrame`), orbit-relative (`RTN`, `LVLH`, `NTW`, `TNW`, `PQW`, `EQW`, `SEZ`, `VNC`, `NSW`), and body/sensor (`BodyFrame`) frames, bound to a string `ObjectId` rather than a NAIF or NORAD ID; `ReferenceFrame::body`/`ReferenceFrame::orbit_relative` construct the unbound label form used before an object identity is known. [@duncaneddy](https://github.com/duncaneddy) ([#508](https://github.com/duncaneddy/brahe/pull/508))

...
def test_download_with_output_path(naif_cache_setup):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        kernel_path = bh.datasets.naif.download_spice_kernel("de440s", output_path)
...
def test_caching_behavior(naif_cache_setup):
    ...
    # First download
    kernel_path1 = bh.datasets.naif.download_spice_kernel("de440s")
    mtime1 = os.path.getmtime(kernel_path1)

    # Second download - should use cache
    kernel_path2 = bh.datasets.naif.download_spice_kernel("de440s")
...
def test_supported_kernels():
    ...
    for kernel in ["de430", "de440s"]:
        ...
        try:
            kernel_path = bh.datasets.naif.download_spice_kernel(kernel)
            assert os.path.exists(kernel_path)

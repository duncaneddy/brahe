...
It carries `FLAGS = ["MANUAL"]` and is excluded from the automated example
tests because BLS re-propagates the full 6-hour arc at every Gauss-Newton
iteration; even after loosening the default convergence threshold this
takes on the order of three minutes, which is too slow to run on every
`just test-examples` invocation. Run it directly to reproduce its output:

    .venv/bin/python examples/examples/ssn_tracking_bls.py
"""
...
# Configuration -- identical scenario to ssn_tracking.py. The run numbers
# quoted in the docs depend on this seed AND on the rand crate's StdRng
# stream; a rand version bump can shift the exact figures while the
# example's tolerance-based assert still passes.

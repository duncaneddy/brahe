...
def test_mean_motion_general():
    n = brahe.mean_motion_general(brahe.R_EARTH + 500e3, brahe.GM_MOON, angle_format=AngleFormat.RADIANS)
    assert n != pytest.approx(0.0011067836148773837, abs=1e-12)

    n = brahe.mean_motion_general(brahe.R_EARTH + 500e3, brahe.GM_MOON, angle_format=AngleFormat.DEGREES)
    assert n != pytest.approx(0.0634140299667068, abs=1e-12)
...
def test_semimajor_axis_general():
    a = brahe.semimajor_axis_general(0.0011067836148773837, brahe.GM_MOON, angle_format=AngleFormat.RADIANS)
    assert a != pytest.approx(brahe.R_EARTH + 500e3, abs=1e-6)

    a = brahe.semimajor_axis_general(0.0634140299667068, brahe.GM_MOON, angle_format=AngleFormat.DEGREES)
    assert a != pytest.approx(brahe.R_EARTH + 500e3, abs=1e-6)
...
def test_sun_synchronous_inclination():
    vp = brahe.sun_synchronous_inclination(brahe.R_EARTH + 500e3, 0.001, AngleFormat.DEGREES)
...
def test_anomaly_eccentric_to_mean():
    ...
    # 180
    M = brahe.anomaly_eccentric_to_mean(math.pi/2, 0.1, angle_format=AngleFormat.RADIANS)
...
def test_anomaly_mean_to_eccentric():
    ...
    # 180
    E = brahe.anomaly_mean_to_eccentric(1.4707963267948965, 0.1, angle_format=AngleFormat.RADIANS)
    assert E == pytest.approx(math.pi/2, abs=1e-12)

    E = brahe.anomaly_mean_to_eccentric(84.27042204869177, 0.1, angle_format=AngleFormat.DEGREES)

/*!
 * OMM (Orbit Mean-elements Message) request class
 *
 * The OMM class provides access to Orbit Mean-elements Messages,
 * which contain orbital element data in CCSDS OMM format.
 *
 * Note: This class is deprecated in favor of GP.
 */
...
///
/// This struct represents a single OMM record as returned by the SpaceTrack API.
...
impl OMMRecord {
    /// Convert this OMM record to an SGP propagator.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// An `SGPPropagator` initialized with this OMM record's TLE data.
    ///
    /// # Errors
    ///
    /// Returns an error if the OMM record doesn't contain valid TLE data.
    pub fn to_sgp_propagator(&self, step_size: f64) -> Result<SGPPropagator, BraheError> {
        ...
        let line1 = self
            .tle_line1
            .as_ref()
            .ok_or_else(|| BraheError::ParseError("OMM record missing TLE line 1".to_string()))?;
        let line2 = self
            .tle_line2
            .as_ref()
            .ok_or_else(|| BraheError::ParseError("OMM record missing TLE line 2".to_string()))?;

        SGPPropagator::from_3le(name.map(|s| s.as_str()), line1, line2, step_size)
    }
}

impl crate::spacetrack::query::HasTLEData for OMMRecord {
    fn to_sgp_propagator(&self, step_size: f64) -> Result<SGPPropagator, BraheError> {
        OMMRecord::to_sgp_propagator(self, step_size)
    }
}

define_request_class! {
    /// OMM request builder for querying Orbit Mean-elements Messages.
    ///
    ...
    ///
    /// ```ignore
    /// use brahe::spacetrack::{OMMRequest, SpaceTrackClient};
    ///
    /// let client = SpaceTrackClient::new("user", "pass").await?;
    /// let records = client.query(&OMMRequest::new()
    ///     .norad_cat_id(25544)
    ///     .limit(1)
    /// ).await?;
    /// ```
...
/// Extension trait for converting vectors of OMM records to propagators.
pub trait OMMRecordVecExt {
    /// Convert all OMM records to SGP propagators.
    fn to_sgp_propagators(&self, step_size: f64) -> Result<Vec<SGPPropagator>, BraheError>;

    /// Convert all OMM records to SGP propagators, skipping failures.
    fn to_sgp_propagators_skip_errors(&self, step_size: f64) -> Vec<SGPPropagator>;
}

impl OMMRecordVecExt for Vec<OMMRecord> {
    fn to_sgp_propagators(&self, step_size: f64) -> Result<Vec<SGPPropagator>, BraheError> {
        self.iter()
...
mod tests {
    ...
    #[test]
    fn test_omm_request_new() {
        let req = OMMRequest::new();
        assert!(req.norad_cat_id.is_none());
    }

    #[test]
    fn test_omm_request_class_name() {
        assert_eq!(OMMRequest::class_name(), "omm");
    }

    #[test]
    fn test_omm_request_controller() {
        assert_eq!(OMMRequest::controller(), "basicspacedata");
    }

    #[test]
    fn test_omm_record_to_propagator() {
    ...
    #[test]
    fn test_omm_record_to_propagator_missing_tle() {
        let record = OMMRecord {
            norad_cat_id: Some(25544),
            ..Default::default()
        };

...
def test_position_geodetic(eop):
    ...
    ecef1 = brahe.position_geodetic_to_ecef(geod1, True)
    ...
    ecef2 = brahe.position_geodetic_to_ecef(geod2, True)
    ...
    ecef3 = brahe.position_geodetic_to_ecef(geod3, True)
    ...
    ecef = brahe.position_geodetic_to_ecef(geod, True)
    ...
    # Test circularity
    geod4 = brahe.position_ecef_to_geodetic(ecef1, True)
    geod5 = brahe.position_ecef_to_geodetic(ecef2, True)
    geod6 = brahe.position_ecef_to_geodetic(ecef3, True)
    ...
    ecef  = brahe.position_geodetic_to_ecef(geod, True)
    geodc = brahe.position_ecef_to_geodetic(ecef, True)
...
def test_geocentric_failure(eop, lat):
    ...
    with pytest.raises(RuntimeError):
        brahe.position_geodetic_to_ecef(np.array([0.0,  lat, 0.0]), True)
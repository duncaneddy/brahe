...
pub fn sun_position(epc: Epoch) -> Vector3<f64> {
    ...
    let L = 2.0 * pi * (0.7859444 + M / (2.0 * pi) + (6892.0 * M.sin() + 72.0 * (2.0 * M).sin()) / 1296.0e3).fract(); // [rad]
...
mod tests {
    ...
    fn test_sun_position(#[case] mjd_tt: f64, #[case] px: f64, #[case] py: f64, #[case] pz: f64) {
        ...
        assert_abs_diff_eq!(epc.mjd_as_time_system(TimeSystem::TT), mjd_tt, epsilon = 1e-9);

...
/// Propagator-owned tidal geopotential field (IERS TN36 Ch. 6): the static
/// gravity coefficients (when spherical-harmonic gravity is configured) plus
/// all enabled tidal ΔC̄nm/ΔS̄nm corrections, folded into one packed Clenshaw
/// table and evaluated in a single pass. Exact by linearity of the
/// geopotential in its coefficients.
///
/// Shared behind `Arc<Mutex<...>>` by the dynamics closure, like
/// [`RotationCache`]; per-epoch delta computation is skipped when consecutive
/// integrator stages share an epoch (`last_t`, keyed on the relative time's
/// bit pattern like the rotation cache).
struct TideField {
    ...
    /// Relative-time key of the last fold-in (bit pattern, `RotationCache`-style).
    last_t: Option<u64>,
}
...
impl DNumericalOrbitPropagator {
    ...
    fn compute_dynamics(
        ...
        if let Some(field) = tide_field {
            let r_ecef = r_i2b * r;
            // TODO(kept from previous implementation): when third_body uses a
            // high-precision ephemeris (DE440/DE440s), source Sun/Moon here
            // from the same provider for per-step consistency. Low-precision is
...
mod tests {
    ...
    fn test_dnumericalorbitpropagator_disable_stm_propagation() {
        ...
        let mut reference = DNumericalOrbitPropagator::new(
            epoch,
            state,
            NumericalPropagationConfig::default(),
            ForceModelConfig::two_body_gravity(),

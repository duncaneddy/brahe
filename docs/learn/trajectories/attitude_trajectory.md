...
- **`Slerp`** (default): spherical linear interpolation of the bracketing quaternions. Exact for constant-angular-rate motion, and always produces a unit quaternion. This is the same interpolation family used for attitude in most spacecraft dynamics software, and matches the wire semantics of an AEM segment with no `INTERPOLATION_METHOD` set.

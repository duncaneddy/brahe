...
use crate::spice::{SPICEKernel, sun_position_spice};
    ...
    match source {
        ...
        Some(source) => {
            let kernel = SPICEKernel::try_from(source)
                .expect("DE ephemeris source should map to a DE kernel");
            sun_position_spice(t, kernel).unwrap_or_else(|_| sun_position(t))
        }
    }
}

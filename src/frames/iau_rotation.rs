 ...
 * (degrees) are the body system's nutation-precession angles. The
 * resulting DCM rotating ICRF vectors into the body-fixed frame is the
 * 3-1-3 sequence `R = Rz(W) * Rx(90 - delta) * Rz(90 + alpha)`.
 *
 * # Known limitations
 *
 * - The embedded `IauRotationModel::nut_prec_angles` table stores only
 *   the linear (`[deg, deg/century]`) term of each nutation-precession
 *   angle. `pck00011.tpc` defines one angle (Mars system angle index 4,
 *   the Phobos longitude `M1`) with an additional quadratic term
 *   (`+12.711923222 deg/century^2`) that is used by Phobos' prime
 *   meridian. Dropping it introduces a slowly growing error in Phobos'
 *   prime meridian angle (of order 0.06 deg by 2050); Mars, Deimos, and
 *   all other bodies in this table do not reference that angle and are
 *   unaffected.
...
/// deg/century]` pairs for the body system's nutation-precession angles;
/// `ra_nut_prec`, `dec_nut_prec`, and `pm_nut_prec` are the sine/cosine
/// amplitude coefficients (degrees) applied to those angles, indexed from
/// the start of `nut_prec_angles` (a coefficient array may be shorter
/// than `nut_prec_angles` — only its own length is evaluated — but is not
/// re-based, so leading zero coefficients present in the source PCK must
/// be preserved to keep indices aligned).
...
pub(crate) struct IauRotationModel {
    ...
    pub pm: [f64; 3],
    /// Nutation-precession angles `[deg, deg/century]` for the body's system.
    pub nut_prec_angles: &'static [[f64; 2]],
...
/// deg/century]`, shared by Mars (499), Phobos (401), and Deimos (402).
/// Transcribed from `pck00011.tpc`'s `BODY4_NUT_PREC_ANGLES` (the
/// "current values" / quadratic-phase block guarded by
/// `BODY4_MAX_PHASE_DEGREE = 2`), truncated to the 26 rows (indices 0-25)
/// referenced by RA/DEC/PM coefficients below; rows 26-30 of the file's
/// 31-row table are not used by any body in this table. The file's third
/// (quadratic-in-T) column is dropped; see the module-level "Known
/// limitations" note.
const MARS_NUT_PREC_ANGLES: &[[f64; 2]] = &[
    [190.72646643, 15917.10818695],
    [21.46892470, 31834.27934054],
    [332.86082793, 19139.89694742],
    [394.93256437, 38280.79631835],
    [189.63271560, 41215158.1842005],
    [121.46893664, 660.22803474],
    [231.05028581, 660.99123540],
    [251.37314025, 1320.50145245],
    [217.98635955, 38279.96125550],
    [196.19729402, 19139.83628608],
    [198.991226, 19139.4819985],
    [226.292679, 38280.8511281],
    [249.663391, 57420.7251593],
    [266.183510, 76560.6367950],
    [79.398797, 0.5042615],
    [122.433576, 19139.9407476],
    [43.058401, 38280.8753272],
    [57.663379, 57420.7517205],
    [79.476401, 76560.6495004],
    [166.325722, 0.5042615],
    [129.071773, 19140.0328244],
    [36.352167, 38281.0473591],
    [56.668646, 57420.9295360],
    [67.364003, 76560.2552215],
    [104.792680, 95700.4387578],
    [95.391654, 0.5042615],
];

/// Jupiter-system (barycenter 5) nutation-precession angles Ja-Je and
/// J1-J10, `[deg, deg/century]`, shared by Jupiter (599), Io (501),
/// Europa (502), Ganymede (503), and Callisto (504). Transcribed from
/// `pck00011.tpc`'s `BODY5_NUT_PREC_ANGLES`.
const JUPITER_NUT_PREC_ANGLES: &[[f64; 2]] = &[
    [73.32, 91472.9],
    [24.62, 45137.2],
    [283.90, 4850.7],
    [355.80, 1191.3],
...
/// deg/century]`, used by the Moon's (301) low-precision IAU rotation
/// model. Transcribed from `pck00011.tpc`'s `BODY3_NUT_PREC_ANGLES`.
const EARTH_MOON_NUT_PREC_ANGLES: &[[f64; 2]] = &[
    [125.045, -1935.5364525000],
...
/// deg/century]`. Transcribed from `pck00011.tpc`'s
/// `BODY8_NUT_PREC_ANGLES` (only the first row is referenced by
/// Neptune's (899) RA/DEC/PM coefficients).
const NEPTUNE_NUT_PREC_ANGLES: &[[f64; 2]] = &[[357.85, 52.316]];

/// Mercury-system (barycenter 1) nutation-precession angles M1-M5, `[deg,
/// deg/century]`. Transcribed from `pck00011.tpc`'s
/// `BODY1_NUT_PREC_ANGLES`.
const MERCURY_NUT_PREC_ANGLES: &[[f64; 2]] = &[
...
const MODELS: &[IauRotationModel] = &[
    ...
    // fifth coefficient multiplies Mars-system angle index 4 (M1), whose
    // quadratic term is dropped; see the module-level "Known
    // limitations" note.
    IauRotationModel {
        naif_id: 401,
...
fn eval(model: &IauRotationModel, epc: Epoch) -> (f64, f64, f64, f64, f64, f64) {
    ...
    let thetas: Vec<(f64, f64)> = model
        .nut_prec_angles
        .iter()
        .map(|[a0, a1]| (a0 + a1 * t, a1 / SECONDS_PER_CENTURY))
        .collect();

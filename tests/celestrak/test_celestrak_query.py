...
class TestCelestrakQueryGP:
    ...
    def test_gp_with_format(self):
        query = bh.celestrak.CelestrakQuery.gp.group("stations").format(
...
class TestCelestrakQuerySupGP:
    ...
    def test_sup_gp_with_format(self):
        query = bh.celestrak.CelestrakQuery.sup_gp.source(
            bh.celestrak.SupGPSource.STARLINK
...
class TestCelestrakQuerySATCAT:
    ...
    def test_satcat_multiple_flags(self):
        query = (
            bh.celestrak.CelestrakQuery.satcat.active(True)
            .payloads(True)
            .on_orbit(True)
        )
        url = query.build_url()
...
class TestCelestrakQueryClientSide:
    ...
    def test_filter(self):
        query = bh.celestrak.CelestrakQuery.gp.group("active").filter(
            "INCLINATION", ">50"
        )
    ...
    def test_order_by(self):
        query = bh.celestrak.CelestrakQuery.gp.group("active").order_by(
            "INCLINATION", False
        )
        assert "GROUP=active" in query.build_url()
    ...
    def test_filter_with_operators(self):
        ...
        query = (
            bh.celestrak.CelestrakQuery.gp.group("active")
...
class TestCelestrakQueryImmutability:
    ...
    def test_chaining(self):
        ...
        query = (
            bh.celestrak.CelestrakQuery.gp.group("stations")
            .format(bh.celestrak.CelestrakOutputFormat.JSON)

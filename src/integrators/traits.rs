...
impl VarmatConfig {
    ...
    // ========================================================================
    // Dynamic (runtime-sized) vector versions
    // ========================================================================

    /// Compute perturbation offsets for each state component (dynamic version).
    fn compute_offsets_dynamic(&self, state: &DVector<f64>) -> DVector<f64> {
        match &self.perturbation {
            PerturbationStrategy::Adaptive {
                scale_factor,
                min_threshold,
    ...
    /// Compute Jacobian using forward finite differences (dynamic version).
    ///
    /// Cost: dimension + 1 function evaluations
    fn compute_forward_dynamic(
        &self,
        t: f64,
        state: DVector<f64>,
        f: &dyn Fn(f64, DVector<f64>) -> DVector<f64>,
        offsets: &DVector<f64>,
    ) -> DMatrix<f64> {
        let dimension = state.len();

        // Evaluate unperturbed dynamics
        let f0 = f(t, state.clone());
    ...
    /// Compute Jacobian using central finite differences (dynamic version).
    ///
    /// Cost: 2*dimension function evaluations (more accurate than forward differences)
    fn compute_central_dynamic(
        &self,
        t: f64,
        state: DVector<f64>,
        f: &dyn Fn(f64, DVector<f64>) -> DVector<f64>,
    ...
    /// Compute the state transition matrix (Jacobian) using configured method (dynamic version).
    ///
    /// This is the main entry point for Jacobian computation with dynamic-sized vectors.
    /// It automatically computes appropriate perturbation sizes and applies the selected
    /// finite difference method.
    ///
    /// # Arguments
    /// - `t`: Current time
    /// - `state`: State vector at time t (runtime-sized)
    /// - `f`: State derivative function (dynamics)
    ///
    /// # Returns
    /// Jacobian matrix ∂f/∂x approximated via finite differences
    ...
    ///
    /// let config = VarmatConfig::default();
    /// let dynamics = |t: f64, state: DVector<f64>| {
    ///     let x = state[0];
    ///     let v = state[1];
    ///     DVector::from_vec(vec![v, -x])  // Simple oscillator
    /// };
    ///
    /// let state = DVector::from_vec(vec![1.0, 0.0]);
    /// let jacobian = config.compute_dynamic(0.0, state, &dynamics);
    /// ```
    pub fn compute_dynamic(
        ...
        let offsets = self.compute_offsets_dynamic(&state);

        match self.method {
            DifferenceMethod::Forward => self.compute_forward_dynamic(t, state, f, &offsets),
            DifferenceMethod::Central => self.compute_central_dynamic(t, state, f, &offsets),
        }
    }
}
...
/// Compute variational matrix with custom perturbation offsets (dynamic version).
///
/// Power user function that allows complete control over perturbation sizes for
/// dynamic-sized state vectors. Useful when you have domain-specific knowledge about
/// appropriate step sizes for different state components.
///
/// # Arguments
/// - `t`: Current time
/// - `state`: State vector at time t (runtime-sized)
...
///
/// # Panics
/// Panics if `offsets` dimension doesn't match `state` dimension.
///
/// # Examples
///
/// ```rust
/// use brahe::integrators::{varmat_custom_dynamic, DifferenceMethod};
/// use nalgebra::DVector;
///
/// let dynamics = |t: f64, state: DVector<f64>| state.clone();
/// let state = DVector::from_vec(vec![1.0, 0.0, 1.0, 0.0, 1.0, 0.0]);
///
/// // Different perturbations for position vs velocity
/// let offsets = DVector::from_vec(vec![
///     1.0, 1.0, 1.0,      // position: 1m
///     0.001, 0.001, 0.001 // velocity: 1mm/s
/// ]);
///
/// let jacobian = varmat_custom_dynamic(
///     0.0, state, &dynamics,
///     DifferenceMethod::Central,
///     offsets
/// );
/// ```
pub fn varmat_custom_dynamic(
    t: f64,
    state: DVector<f64>,
    f: &dyn Fn(f64, DVector<f64>) -> DVector<f64>,
    method: DifferenceMethod,
    ...
    assert_eq!(
        state.len(),
        offsets.len(),
        "State and offsets must have the same dimension"
    );
    ...
    match method {
        DifferenceMethod::Forward => config.compute_forward_dynamic(t, state, f, &offsets),
        DifferenceMethod::Central => config.compute_central_dynamic(t, state, f, &offsets),
    }
}

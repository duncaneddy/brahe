# Solar Radiation Pressure

Solar radiation pressure acceleration and eclipse modeling.

!!! note
    For conceptual explanations and examples, see [Solar Radiation Pressure](../../learn/orbital_dynamics/solar_radiation_pressure.md) in the Learn section.

## SRP Acceleration

::: brahe.accel_solar_radiation_pressure

## Eclipse Modeling

::: brahe.eclipse_conical

::: brahe.eclipse_cylindrical

## Arbitrary Occulting Body

::: brahe.eclipse_conical_for_body

::: brahe.eclipse_cylindrical_for_body

## See Also

- [Solar Radiation Pressure (Learn)](../../learn/orbital_dynamics/solar_radiation_pressure.md) - Conceptual explanation and examples
- [Frame Router & Multibody Propagation](../../learn/frames/frame_transformations.md) - `OccultingBody` and non-Earth propagation
- [Orbital Dynamics Module](index.md) - Complete orbit dynamics API reference

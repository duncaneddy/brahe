...
def numerical_twobody(params: dict, iterations: int):
    """RK4 numerical propagation under two-body (point-mass Earth) dynamics."""
    jd_utc = float(params["jd"])
    epoch_utcmjd = jd_utc - _JD_TO_UTCMJD
    step_size = float(params["step_size"])
    n_steps = int(params["n_steps"])
    oe_deg_si = list(params["elements"])  # [a(m), e, i_deg, RAAN_deg, AOP_deg, M_deg]

    # Pre-convert Keplerian IC to Cartesian OUTSIDE timed region (conversion is
    # cheap and epoch-independent for point-mass; avoids polluting timing).
    oe_gmat = _oe_deg_si_to_gmat(oe_deg_si)
...
def numerical_rk4_grav5x5(params: dict, iterations: int):
    """RK4 numerical propagation under JGM-2 5x5 spherical-harmonic gravity.

    Parameters follow the force-model task layout generated by
    ``NumericalRk4Grav5x5Task.generate_params``:
    ...
    step_size = float(params["step_size"])
    n_steps = int(params["n_steps"])
    oe_deg_si = list(params["elements_deg"])

    param_vec = params.get("params", [1000.0, 10.0, 2.2, 10.0, 1.3])
    sc_props = (
        float(param_vec[0]),  # mass (kg)
        float(param_vec[1]),  # drag_area (m^2)
        float(param_vec[2]),  # Cd
        float(param_vec[3]),  # srp_area (m^2)
        float(param_vec[4]),  # Cr
    )

    # Pre-convert Keplerian IC to Cartesian OUTSIDE timed region.
    oe_gmat = _oe_deg_si_to_gmat(oe_deg_si)
    r0_km = _keplerian_to_cartesian_km(oe_gmat)
    ...
    def run():
        ...
        return _propagate(r0_km, epoch_utcmjd, step_size, n_steps,
                          sc_props=sc_props,
                          configure_forces_fn=configure_forces)
...
def numerical_rk4_grav20x20_sun_moon(params: dict, iterations: int):
    """RK4 numerical propagation under JGM-2 20x20 gravity + Sun/Moon third bodies.

    Parameters follow the force-model task layout generated by
    ``NumericalRk4Grav20x20SunMoonTask.generate_params``:
      - jd            : UTC Julian date of epoch
      - elements_deg  : [a (m), e, i_deg, RAAN_deg, AOP_deg, M_deg]
      - step_size     : RK4 step size in seconds
      - n_steps       : number of integration steps
      - params        : [mass (kg), drag_area (m^2), Cd, srp_area (m^2), Cr]
      - gravity_degree: 20 (informational)
      - gravity_order : 20 (informational)
      - third_body_sun : True (informational)
      - third_body_moon: True (informational)
    """
    jd_utc = float(params["jd"])
    epoch_utcmjd = jd_utc - _JD_TO_UTCMJD
    step_size = float(params["step_size"])
    ...
    def configure_forces(ps):
        # Replace the default Earth PointMassForce with a 20x20 GravityField.
        _add_gravity_field(ps, 20, 20, "JGM2.cof")
        # Add third-body point-mass forces.
        _add_third_body_force(ps, "Sun")
        _add_third_body_force(ps, "Luna")
        # Wire coordinate-system references after all forces are added.
        import gmatpy as gmat
        gmat.Initialize()
...
def numerical_rk4_grav80x80_full(params: dict, iterations: int):
    ...
    # Use a mutable counter so each call to run() uses unique object names.
    counter = [0]
...
def keplerian_trajectory(params: dict, iterations: int):
    """Analytical Keplerian propagation — 1 LEO orbit over ~90 time steps.

    Parameters (from KeplerianTrajectoryTask.generate_params):
      - jd        : UTC Julian date of epoch
      - elements  : [a (m), e, i_deg, RAAN_deg, AOP_deg, M_deg]
      - step_size : step size in seconds (60.0)
      - n_steps   : number of steps (~90, one orbital period)

    GMAT's Type='Keplerian' PropSetup provides analytical two-body propagation.
    """
    jd_utc = float(params["jd"])
    epoch_utcmjd = jd_utc - _JD_TO_UTCMJD
    oe_deg_si = list(params["elements"])
    step_size = float(params["step_size"])
    n_steps = int(params["n_steps"])
    ...
    def run():
        gmat_clear()
        return _propagate_keplerian_trajectory(r0_km, epoch_utcmjd, step_size, n_steps)

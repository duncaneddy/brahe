...
def run_accuracy(
    ...
    with (
        open(archival_path, "w") as archival_handle,
        open(latest_path, "w") as latest_handle,
    ):
...
def _dispatch_one(task: BenchmarkTask, language: str, params: dict):
    ...
    if language == "java":
        from benchmarks.comparative.runner import _ensure_java_home, _get_java_command
...
def _run_subprocess(
    task: BenchmarkTask, language: str, input_data: dict, command: list[str] | None
):
    if command is None:
        return None
    try:
...
def _sample_keys_from_params(task: BenchmarkTask, params: dict, n: int) -> list[dict]:
    ...
    for elem in list_value:
        ...
        except Exception:  # noqa: BLE001 - isolate arbitrary failures of the wrapped implementation
            keys.append({})
    return keys

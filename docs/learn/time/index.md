...
| `TDB`      | `TDB` is Barycentric Dynamical Time, the time scale for solar system barycentric ephemerides. It differs from `TT` by small periodic terms (< 1.7 ms) arising from the relativistic effects of Earth's orbital motion. Seconds past J2000 in `TDB` is SPICE ephemeris time (ET).                    |
| `TCG`      | `TCG` is Geocentric Coordinate Time, the coordinate time for geocentric reference systems. It differs from `TT` by a secular drift of roughly 22 milliseconds per year caused by Earth's gravitational time dilation.                                                                                 |
| `TCB`      | `TCB` is Barycentric Coordinate Time, the coordinate time for the solar system barycenter. It differs from `TDB` by a secular drift arising from relativistic effects.                                                                                                                                |
| `BDT`      | `BDT` is BeiDou Navigation Satellite System Time. It was aligned with `UTC` at its inception on January 1, 2006 and holds a fixed offset from `TAI` of 33 seconds (`BDT` = `TAI` - 33s).                                                                                                              |
| `GST`      | `GST` is Galileo System Time, used by the Galileo navigation system. It is steered to `GPS` time and shares the same fixed offset from `TAI` of 19 seconds (`GST` = `TAI` - 19s).                                                                                                                     |

...
use crate::relative_motion::{
    omega_enz, omega_lvlh, omega_nsw, omega_ntw_for_body, omega_rtn, omega_sez, omega_tnw_for_body,
    omega_vnc_for_body, rotation_ecef_to_enz, rotation_ecef_to_sez, rotation_eci_to_eqw,
    rotation_eci_to_lvlh, rotation_eci_to_nsw, rotation_eci_to_ntw, rotation_eci_to_rtn,
...
pub(crate) struct Resolved {
    ...
    pub(crate) rateless_link: Option<ReferenceFrame>,
}
...
fn chain_rate(frame: &ReferenceFrame, resolved: &Resolved) -> Result<Vector3<f64>, BraheError> {
...
pub(crate) fn resolve_orientation(
    ...
    match frame {
        ReferenceFrame::Celestial(celestial) => Ok(Resolved {
            root: *celestial,
...
fn unbound_frame_error(frame: &ReferenceFrame) -> BraheError {
    let fix = match frame {
        ReferenceFrame::OrbitRelative { kind, .. } => format!("construct with ReferenceFrame::{kind}(object)"),
        _ => "construct with an object (e.g. ReferenceFrame::SC_BODY(\"SC\")) or bind via register_for"
            .to_string(),
    };
...
fn resolve_body(
    ...
    let mut rateless_link: Option<ReferenceFrame> = None;
...
fn provider_error(frame: &ReferenceFrame, err: BraheError) -> BraheError {
    BraheError::Error(format!("orientation for {frame}: {err}"))
}

/// Resolves a topocentric orbit-relative kind's site state in ITRF, guarding
/// that the object is Earth-centered.
///
/// `SEZ` and `ENZ` are defined only for Earth-centered objects, since their
/// axes come from the WGS84 geodetic vertical. When the object's state is
/// already declared in ITRF, `x` is used directly rather than round-tripping
/// `x_root` back through `state_frame_to_frame`.
///
/// # Arguments
/// - `kind`: The topocentric orbit-relative kind being resolved (used to
///   name the frame in the error message)
/// - `object`: The object the frame is bound to
/// - `declared`: The celestial frame the object's state is declared in
/// - `root`: The ICRF-aligned inertial frame sharing `declared`'s center
/// - `x`: The object's state as declared, in `declared` axes
/// - `x_root`: The object's state converted into `root` axes
/// - `epc`: Epoch instant for evaluation
///
/// # Returns
...
fn earth_site_state(
    ...
    if root.center() != FrameCenter::Body(NAIFId::Earth) {
        return Err(BraheError::Error(format!(
            "orbit-relative kind {kind} is a topocentric frame on the WGS84 ellipsoid and \
             requires an Earth-centered object; {object} is declared in {declared}"
        )));
    }
    if declared == CelestialFrame::ITRF {
        Ok(x)
    } else {
...
///   `SEZ` or `ENZ`) `object`'s declared center is not Earth
        ...
        OrbitRelativeFrameKind::TNW => {
            let omega = match variant {
                OrbitRelativeFrameVariant::Rotating => {
                    let gm = body_gm(root.center().naif_id()).map_err(|e| {
                        BraheError::Error(format!(
                            "orbit-relative kind TNW needs the gravitational parameter of \
                             {object}'s declared center for its rotating rate: {e}"
                        ))
                    })?;
                    omega_tnw_for_body(x_root, gm)
                }
                OrbitRelativeFrameVariant::Inertial => Vector3::zeros(),
            };
            (rotation_eci_to_tnw(x_root), omega)
        }
        OrbitRelativeFrameKind::VNC => {
            let omega = match variant {
                OrbitRelativeFrameVariant::Rotating => {
                    let gm = body_gm(root.center().naif_id()).map_err(|e| {
                        BraheError::Error(format!(
                            "orbit-relative kind VNC needs the gravitational parameter of \
                             {object}'s declared center for its rotating rate: {e}"
                        ))
                    })?;
                    omega_vnc_for_body(x_root, gm)
                }
                OrbitRelativeFrameVariant::Inertial => Vector3::zeros(),
            };
...
impl ReferenceFrame {
    ...
    pub(crate) fn origin(&self) -> Result<Origin, BraheError> {
        match self {
            ReferenceFrame::Celestial(celestial) => Ok(Origin::Celestial(*celestial)),
            ReferenceFrame::Body {
                object: Some(object),
                ..
            }
            ...
            ReferenceFrame::Body { object: None, .. }
            | ReferenceFrame::OrbitRelative { object: None, .. } => Err(unbound_frame_error(self)),
        }
    }
}
...
mod tests {
    ...
    fn test_body_chain_matches_manual_composition() {
        ...
        .unwrap();
        let r = rotation_frame_to_frame(CelestialFrame::GCRF, ReferenceFrame::CSS("SC", "1"), epc)
            .unwrap();
        let expected =
        ...
        let r_inv =
            rotation_frame_to_frame(ReferenceFrame::CSS("SC", "1"), CelestialFrame::GCRF, epc)
                .unwrap();
    ...
    fn test_body_chain_composes_angular_velocity() {
        ...
        let resolved = resolve_orientation(&ReferenceFrame::CSS("SC", "1"), epc, true).unwrap();
    ...
    fn test_body_chain_angular_velocity_none_when_link_lacks_rates() {
        ...
        assert!(
            resolve_orientation(&ReferenceFrame::CSS("SC", "1"), epc, true)
                .unwrap()
                .omega
                .is_none()
        );
    ...
    fn test_rotation_and_position_do_not_evaluate_angular_velocity() {
        ...
        assert!(
            rotation_frame_to_frame(CelestialFrame::GCRF, ReferenceFrame::SC_BODY("SC"), epc)
                .is_ok()
        );
        assert!(
    ...
    fn test_cross_root_rotation_joins_chains_through_celestial() {
        ...
        let r = rotation_frame_to_frame(CelestialFrame::GCRF, ReferenceFrame::SC_BODY("SC"), epc)
            .unwrap();
        let expected =
            rotation_frame_to_frame(CelestialFrame::GCRF, CelestialFrame::ITRF, epc).unwrap();
    ...
    fn test_rtn_variants_angular_velocity() {
        ...
        let rotating = resolve_orientation(&ReferenceFrame::RTN("A"), epc, true).unwrap();
    ...
    #[test]
    #[serial]
    fn test_tnw_rotation_and_rate_match_relative_motion() {
        clear_object_registry();
        let epc = Epoch::from_date(2024, 3, 1, TimeSystem::UTC);
        let oe = SVector6::new(R_EARTH + 500e3, 0.05, 97.8, 15.0, 30.0, 45.0);
        let x = state_koe_to_eci(oe, AngleFormat::Degrees);
        register_object("A", FnProvider(move |_| Ok(x)), CelestialFrame::GCRF).unwrap();
        let r =
            rotation_frame_to_frame(CelestialFrame::GCRF, ReferenceFrame::TNW("A"), epc).unwrap();
        assert_abs_diff_eq!(r, rotation_eci_to_tnw(x), epsilon = 1e-14);
        let resolved = resolve_orientation(&ReferenceFrame::TNW("A"), epc, true).unwrap();
        assert_abs_diff_eq!(resolved.omega.unwrap(), omega_tnw(x), epsilon = 1e-15);
        clear_object_registry();
    }

    #[test]
    #[serial]
    fn test_vnc_rate_uses_the_declared_center_gm() {
        clear_object_registry();
        let epc = Epoch::from_date(2024, 3, 1, TimeSystem::UTC);
        let x = state_koe_to_inertial_for_body(
            SVector6::new(R_MARS + 400e3, 0.05, 92.6, 45.0, 270.0, 10.0),
        ...
        let mars_icrf = CelestialFrame::centered(499, FrameAxes::ICRF);
        register_object("M", FnProvider(move |_| Ok(x)), mars_icrf).unwrap();
        let resolved = resolve_orientation(&ReferenceFrame::VNC("M"), epc, true).unwrap();
        assert_abs_diff_eq!(
            resolved.omega.unwrap(),
            omega_vnc_for_body(x, GM_MARS),
            epsilon = 1e-15
        );
        clear_object_registry();
    }
    ...
    #[test]
    #[serial]
    fn test_vnc_inertial_variant_needs_no_gm() {
        clear_object_registry();
        let epc = Epoch::from_date(2024, 3, 1, TimeSystem::UTC);
        // NAIF ID 2000001 has no packaged GM constant, so it only works for
        // the inertial variant, which does not need one.
        ...
        let err = rotation_frame_to_frame(CelestialFrame::GCRF, ReferenceFrame::VNC("A"), epc)
            .unwrap_err()
            .to_string();
        assert!(err.contains("VNC"));
        assert!(err.contains("gravitational parameter"));
        clear_object_registry();
    }
    ...
    #[test]
    #[serial]
    fn test_vnc_rotation_and_rate_match_relative_motion() {
        clear_object_registry();
        let epc = Epoch::from_date(2024, 3, 1, TimeSystem::UTC);
        let oe = SVector6::new(R_EARTH + 500e3, 0.05, 97.8, 15.0, 30.0, 45.0);
        let x = state_koe_to_eci(oe, AngleFormat::Degrees);
        register_object("A", FnProvider(move |_| Ok(x)), CelestialFrame::GCRF).unwrap();
        let r =
            rotation_frame_to_frame(CelestialFrame::GCRF, ReferenceFrame::VNC("A"), epc).unwrap();
        assert_abs_diff_eq!(r, rotation_eci_to_vnc(x), epsilon = 1e-14);
        let resolved = resolve_orientation(&ReferenceFrame::VNC("A"), epc, true).unwrap();
        assert_abs_diff_eq!(resolved.omega.unwrap(), omega_vnc(x), epsilon = 1e-15);
        clear_object_registry();
    }
    ...
    fn test_rtn_non_icrf_declared_frame_converts_to_inertial_root() {
        ...
        let resolved = resolve_orientation(&ReferenceFrame::RTN("A"), epc, true).unwrap();
        assert_eq!(resolved.root, CelestialFrame::GCRF);
    ...
    #[test]
    #[serial]
    fn test_enz_station_rotation_matches_relative_motion() {
        setup_global_test_eop();
        clear_object_registry();
        let epc = Epoch::from_date(2024, 3, 1, TimeSystem::UTC);
        let r = position_geodetic_to_ecef(Vector3::new(30.0, 45.0, 500.0), AngleFormat::Degrees)
        ...
        // Rotating variant: rate relative to GCRF is Earth's rotation in ENZ axes
    ...
    #[test]
    #[serial]
    fn test_enz_requires_earth_centered_object() {
        clear_object_registry();
        let epc = Epoch::from_date(2024, 3, 1, TimeSystem::UTC);
        let x = SVector6::new(R_MARS + 400e3, 0.0, 0.0, 0.0, 3.4e3, 0.0);
        register_object(
        ...
        .unwrap();
        let err = rotation_frame_to_frame(CelestialFrame::GCRF, ReferenceFrame::ENZ("M"), epc)
            .unwrap_err()
            .to_string();
        assert!(err.contains("ENZ"), "{err}");
        assert!(err.contains("Earth"), "{err}");
        clear_object_registry();
    }
    ...
    fn test_provider_failure_is_wrapped_with_frame_context() {
        ...
        .unwrap();
        let err = rotation_frame_to_frame(CelestialFrame::GCRF, ReferenceFrame::SC_BODY("SC"), epc)
            .unwrap_err()
            .to_string();
    ...
    fn test_sun_vector_in_sensor_frame() {
        ...
        let got = position_frame_to_frame(
            CelestialFrame::GCRF,
            ReferenceFrame::CSS("SC", "1"),
            epc,
            sun_gcrf,
    ...
    fn test_object_origin_offset_crosses_centers() {
        ...
        let x_rtn =
            state_frame_to_frame(CelestialFrame::GCRF, ReferenceFrame::RTN("A"), epc, x_b).unwrap();
        let via_rtn =
            state_frame_to_frame(ReferenceFrame::RTN("A"), CelestialFrame::LCI, epc, x_rtn)
                .unwrap();
        let direct =
        ...
        let p_rtn =
            position_frame_to_frame(CelestialFrame::GCRF, ReferenceFrame::RTN("A"), epc, p_b)
                .unwrap();
        let p_via =
            position_frame_to_frame(ReferenceFrame::RTN("A"), CelestialFrame::LCI, epc, p_rtn)
                .unwrap();
        let p_direct =
    ...
    fn test_same_frame_short_circuits_without_evaluation() {
        ...
        clear_object_registry();
        let epc = Epoch::from_date(2024, 3, 1, TimeSystem::UTC);
        let frame = ReferenceFrame::CSS("SC", "1");
    ...
    fn test_cross_root_state_transport_round_trips() {
        ...
        let x_body = state_frame_to_frame(
            CelestialFrame::GCRF,
            ReferenceFrame::SC_BODY("SC"),
            epc,
            x_gcrf,

...
mod tests {
    ...
    #[test]
    #[serial_test::parallel]
    fn test_rkf45d_step_methods_require_dt() {
        let integrator = RKF45DIntegrator::new(1, Box::new(simple_decay_d), None, None, None);
        let state = DVector::from_vec(vec![1.0]);
    ...
    #[test]
    #[serial_test::parallel]
    fn test_rkf45s_control_input_applied() {
        ...
        let integrator: RKF45SIntegrator<1, 0> =
            RKF45SIntegrator::new(Box::new(f), None, None, Some(Box::new(control)));
        let state = SVector::<f64, 1>::new(0.0);
    ...
    #[test]
    #[serial_test::parallel]
    fn test_rkf45d_control_input_applied() {
        let f = |_t: f64,
                 x: &DVector<f64>,
        ...
        let integrator = RKF45DIntegrator::new(1, Box::new(f), None, None, Some(Box::new(control)));
        let state = DVector::from_vec(vec![0.0]);
        let result = integrator.step(0.0, state, None, Some(0.1)).unwrap();
        assert!(result.state[0] > 0.0);
    }
    ...
    #[test]
    #[serial_test::parallel]
    fn test_rkf45s_step_with_sensmat_requires_sensmat() {
        ...
        let integrator: RKF45SIntegrator<1, 0> = RKF45SIntegrator::new(
    ...
    #[test]
    #[serial_test::parallel]
    fn test_rkf45d_step_with_sensmat_requires_sensmat() {
        let jacobian = DNumericalJacobian::new(Box::new(simple_decay_d));
        let integrator = RKF45DIntegrator::new(
            1,

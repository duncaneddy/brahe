...
def test_batch_ntw_covariance_preserves_state_batch_shape(eop):
    variant = brahe.OrbitRelativeFrameVariant.ROTATING
    states = np.array([_eccentric_state(dt=60.0 * i) for i in range(4)]).reshape(
        2, 2, 6
    )
    p = np.eye(6)
    p_out = brahe.covariance_ntw_to_eci(states, p, variant)
    assert p_out.shape == (2, 2, 6, 6)
    for i in range(2):
        for j in range(2):
            np.testing.assert_array_equal(
                p_out[i, j], brahe.covariance_ntw_to_eci(states[i, j], p, variant)
            )

    x = _eccentric_state()
    covs = np.array([np.eye(6) * (i + 1.0) for i in range(3)])
    p_single_state = brahe.covariance_eci_to_ntw(x, covs, variant)
    assert p_single_state.shape == (3, 6, 6)
    for k in range(3):
        np.testing.assert_array_equal(
            p_single_state[k], brahe.covariance_eci_to_ntw(x, covs[k], variant)
        )

    batch = np.array([_eccentric_state(dt=60.0 * i) for i in range(3)])
    p_singleton_batch = brahe.covariance_ntw_to_eci(batch[:1], covs, variant)
    assert p_singleton_batch.shape == (3, 6, 6)
    for k in range(3):
        np.testing.assert_array_equal(
            p_singleton_batch[k],
            brahe.covariance_ntw_to_eci(batch[0], covs[k], variant),
        )


def test_batch_ntw_empty_covariance_batch(eop):
    variant = brahe.OrbitRelativeFrameVariant.ROTATING
    x = _eccentric_state()
    empty = brahe.covariance_ntw_to_eci(x, np.zeros((0, 6, 6)), variant)
    assert empty.shape == (0, 6, 6)
    with pytest.raises(ValueError, match="6x6"):
        brahe.covariance_ntw_to_eci(x, np.zeros((0, 5, 5)), variant)

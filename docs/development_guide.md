...
not by hand. Examples flagged `NETWORK` fetch data that cannot be served from
the warmed CI caches (e.g. GCAT, ICGEM, NAIF, star-catalog, Horizons, and
SBDB lookups) and are skipped by default so `just test-examples` works
...
figures from being generated. Celestrak-backed examples are not flagged
`NETWORK`; they run from the warmed Celestrak cache on every PR.

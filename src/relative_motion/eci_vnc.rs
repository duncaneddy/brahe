...
/// Computes the VNC-to-inertial covariance Jacobian, for a body with gravitational parameter
/// `gm`, for each state in `x_inertial`.
///
/// Batch form of [`jacobian_vnc_to_inertial_for_body`]. Evaluation runs on the global thread
...
///
/// # Returns
/// - Jacobians such that `P_inertial = J P_vnc Jᵀ`, one per state, in input order
///
/// # References
/// 1. NASA Conjunction Assessment Risk Analysis (CARA),
///    [*Conjunction Assessment Handbook*, NASA/SP-20205011318, Appendix N (RIC-to-ECI covariance transformation, eq. N-13)](https://ntrs.nasa.gov/citations/20205011318)
...
/// use brahe::constants::GM_MARS;
/// use brahe::frames::OrbitRelativeFrameVariant;
/// use brahe::relative_motion::jacobians_vnc_to_inertial_for_body;
/// use brahe::vector6_from_array;
///
/// let x = vector6_from_array([3.8e6, 0.0, 0.0, 0.0, 3200.0, 0.0]);
/// let j = jacobians_vnc_to_inertial_for_body(&[x, x], GM_MARS, OrbitRelativeFrameVariant::Rotating);
/// assert_eq!(j.len(), 2);
/// ```
pub fn jacobians_vnc_to_inertial_for_body(
    x_inertial: &[SVector6],
    gm: f64,
    variant: OrbitRelativeFrameVariant,
) -> Vec<SMatrix6> {
    batch_map(
        |x| jacobian_vnc_to_inertial_for_body(*x, gm, variant),
        x_inertial,
    )
}

/// Computes the VNC-to-ECI covariance Jacobian for each state in `x_eci`.
///
/// Batch form of [`jacobian_vnc_to_eci`]. Evaluation runs on the global thread pool for
/// large inputs.
///
/// # Arguments
...
///
/// # Returns
/// - Jacobians such that `P_eci = J P_vnc Jᵀ`, one per state, in input order
///
/// # References
/// 1. NASA Conjunction Assessment Risk Analysis (CARA),
///    [*Conjunction Assessment Handbook*, NASA/SP-20205011318, Appendix N (RIC-to-ECI covariance transformation, eq. N-13)](https://ntrs.nasa.gov/citations/20205011318)
...
/// use brahe::coordinates::state_koe_to_eci;
/// use brahe::frames::OrbitRelativeFrameVariant;
/// use brahe::relative_motion::jacobians_vnc_to_eci;
/// use brahe::vector6_from_array;
///
/// let x = state_koe_to_eci(vector6_from_array([R_EARTH + 700e3, 0.001, 97.8, 15.0, 30.0, 45.0]), AngleFormat::Degrees);
/// let j = jacobians_vnc_to_eci(&[x, x], OrbitRelativeFrameVariant::Rotating);
/// assert_eq!(j.len(), 2);
/// ```
pub fn jacobians_vnc_to_eci(
    x_eci: &[SVector6],
    variant: OrbitRelativeFrameVariant,
) -> Vec<SMatrix6> {
    batch_map(|x| jacobian_vnc_to_eci(*x, variant), x_eci)
}

/// Computes the inertial-to-VNC covariance Jacobian, for a body with gravitational parameter
/// `gm`, for each state in `x_inertial`.
///
/// Batch form of [`jacobian_inertial_to_vnc_for_body`]. Evaluation runs on the global thread
/// pool for large inputs.
///
/// # Arguments
/// - `x_inertial`: Cartesian inertial states of the frame's origin (position, velocity). Units: (*m*; *m/s*)
...
///
/// # Returns
/// - Jacobians such that `P_vnc = J P_inertial Jᵀ`, one per state, in input order
///
/// # References
/// 1. NASA Conjunction Assessment Risk Analysis (CARA),
///    [*Conjunction Assessment Handbook*, NASA/SP-20205011318, Appendix N (RIC-to-ECI covariance transformation, eq. N-13)](https://ntrs.nasa.gov/citations/20205011318)
...
/// use brahe::constants::GM_MARS;
/// use brahe::frames::OrbitRelativeFrameVariant;
/// use brahe::relative_motion::jacobians_inertial_to_vnc_for_body;
/// use brahe::vector6_from_array;
///
/// let x = vector6_from_array([3.8e6, 0.0, 0.0, 0.0, 3200.0, 0.0]);
/// let j = jacobians_inertial_to_vnc_for_body(&[x, x], GM_MARS, OrbitRelativeFrameVariant::Rotating);
/// assert_eq!(j.len(), 2);
/// ```
pub fn jacobians_inertial_to_vnc_for_body(
    x_inertial: &[SVector6],
    gm: f64,
    variant: OrbitRelativeFrameVariant,
) -> Vec<SMatrix6> {
    batch_map(
        |x| jacobian_inertial_to_vnc_for_body(*x, gm, variant),
        x_inertial,
    )
}

/// Computes the ECI-to-VNC covariance Jacobian for each state in `x_eci`.
///
/// Batch form of [`jacobian_eci_to_vnc`]. Evaluation runs on the global thread pool for
/// large inputs.
///
/// # Arguments
/// - `x_eci`: Cartesian ECI states of the frame's origin (position, velocity). Units: (*m*; *m/s*)
...
///
/// # Returns
/// - Jacobians such that `P_vnc = J P_eci Jᵀ`, one per state, in input order
///
/// # References
/// 1. NASA Conjunction Assessment Risk Analysis (CARA),
///    [*Conjunction Assessment Handbook*, NASA/SP-20205011318, Appendix N (RIC-to-ECI covariance transformation, eq. N-13)](https://ntrs.nasa.gov/citations/20205011318)
...
/// use brahe::coordinates::state_koe_to_eci;
/// use brahe::frames::OrbitRelativeFrameVariant;
/// use brahe::relative_motion::jacobians_eci_to_vnc;
/// use brahe::vector6_from_array;
///
/// let x = state_koe_to_eci(vector6_from_array([R_EARTH + 700e3, 0.001, 97.8, 15.0, 30.0, 45.0]), AngleFormat::Degrees);
/// let j = jacobians_eci_to_vnc(&[x, x], OrbitRelativeFrameVariant::Rotating);
/// assert_eq!(j.len(), 2);
/// ```
pub fn jacobians_eci_to_vnc(
    x_eci: &[SVector6],
    variant: OrbitRelativeFrameVariant,
) -> Vec<SMatrix6> {
    batch_map(|x| jacobian_eci_to_vnc(*x, variant), x_eci)
}

/// Transforms each state covariance in `covariances` from VNC axes into an inertial frame
/// centered on a body with gravitational parameter `gm`.
///
/// Batch form of [`covariance_vnc_to_inertial_for_body`]. Evaluation runs on the global
...
/// - `covariances`: State covariances in VNC axes, length 1 or the batch length. Units: (*m²*, *m²/s*, *m²/s²*)
/// - `gm`: Gravitational parameter of the central body. Units: (*m³/s²*)
/// - `variant`: Whether the VNC axes rotate with the orbit or are frozen at the epoch
///
/// # Returns
...
/// use brahe::constants::GM_MARS;
/// use brahe::frames::OrbitRelativeFrameVariant;
/// use brahe::relative_motion::covariances_vnc_to_inertial_for_body;
/// use brahe::vector6_from_array;
///
/// let x = vector6_from_array([3.8e6, 0.0, 0.0, 0.0, 3200.0, 0.0]);
/// let p = vec![SMatrix6::identity(); 2];
/// let p_inertial = covariances_vnc_to_inertial_for_body(&[x], &p, GM_MARS, OrbitRelativeFrameVariant::Rotating).unwrap();
/// assert_eq!(p_inertial.len(), 2);
/// ```
pub fn covariances_vnc_to_inertial_for_body(
    x_inertial: &[SVector6],
    ...
    batch_zip(
        |x, p| covariance_vnc_to_inertial_for_body(*x, p, gm, variant),
        x_inertial,
        covariances,
    )
}

/// Transforms each state covariance in `covariances` from VNC axes into ECI axes.
///
/// Batch form of [`covariance_vnc_to_eci`]. Evaluation runs on the global thread pool for
/// large inputs.
///
...
/// use brahe::coordinates::state_koe_to_eci;
/// use brahe::frames::OrbitRelativeFrameVariant;
/// use brahe::relative_motion::covariances_vnc_to_eci;
/// use brahe::vector6_from_array;
///
/// let x = state_koe_to_eci(vector6_from_array([R_EARTH + 700e3, 0.001, 97.8, 15.0, 30.0, 45.0]), AngleFormat::Degrees);
/// let p = vec![SMatrix6::identity(); 2];
/// let p_eci = covariances_vnc_to_eci(&[x], &p, OrbitRelativeFrameVariant::Rotating).unwrap();
/// assert_eq!(p_eci.len(), 2);
/// ```
pub fn covariances_vnc_to_eci(
    x_eci: &[SVector6],
    covariances: &[SMatrix6],
    variant: OrbitRelativeFrameVariant,
) -> Result<Vec<SMatrix6>, BraheError> {
    batch_zip(
        |x, p| covariance_vnc_to_eci(*x, p, variant),
        x_eci,
        covariances,
    )
}
...
/// body with gravitational parameter `gm` into VNC axes.
///
/// Batch form of [`covariance_inertial_to_vnc_for_body`]. Evaluation runs on the global
/// thread pool for large inputs.
///
/// The `x_inertial` and `covariances` arguments follow the broadcast rule: each argument has
/// length 1 or the common batch length.
...
///
/// # Returns
/// - State covariances in VNC axes, in input order. Units: (*m²*, *m²/s*, *m²/s²*)
/// - Error if the lengths do not satisfy the broadcast rule
///
/// # References
/// 1. NASA Conjunction Assessment Risk Analysis (CARA),
...
/// use brahe::constants::GM_MARS;
/// use brahe::frames::OrbitRelativeFrameVariant;
/// use brahe::relative_motion::covariances_inertial_to_vnc_for_body;
/// use brahe::vector6_from_array;
///
/// let x = vector6_from_array([3.8e6, 0.0, 0.0, 0.0, 3200.0, 0.0]);
/// let p = vec![SMatrix6::identity(); 2];
/// let p_vnc = covariances_inertial_to_vnc_for_body(&[x], &p, GM_MARS, OrbitRelativeFrameVariant::Rotating).unwrap();
/// assert_eq!(p_vnc.len(), 2);
/// ```
pub fn covariances_inertial_to_vnc_for_body(
    x_inertial: &[SVector6],
    covariances: &[SMatrix6],
    gm: f64,
    variant: OrbitRelativeFrameVariant,
    ...
    batch_zip(
        |x, p| covariance_inertial_to_vnc_for_body(*x, p, gm, variant),
        x_inertial,
        covariances,
    )
}

/// Transforms each state covariance in `covariances` from ECI axes into VNC axes.
///
/// Batch form of [`covariance_eci_to_vnc`]. Evaluation runs on the global thread pool for
/// large inputs.
///
/// The `x_eci` and `covariances` arguments follow the broadcast rule: each argument has
/// length 1 or the common batch length.
...
/// use brahe::coordinates::state_koe_to_eci;
/// use brahe::frames::OrbitRelativeFrameVariant;
/// use brahe::relative_motion::covariances_eci_to_vnc;
/// use brahe::vector6_from_array;
///
/// let x = state_koe_to_eci(vector6_from_array([R_EARTH + 700e3, 0.001, 97.8, 15.0, 30.0, 45.0]), AngleFormat::Degrees);
/// let p = vec![SMatrix6::identity(); 2];
/// let p_vnc = covariances_eci_to_vnc(&[x], &p, OrbitRelativeFrameVariant::Rotating).unwrap();
/// assert_eq!(p_vnc.len(), 2);
/// ```
pub fn covariances_eci_to_vnc(
    x_eci: &[SVector6],
    covariances: &[SMatrix6],
    variant: OrbitRelativeFrameVariant,
) -> Result<Vec<SMatrix6>, BraheError> {
    batch_zip(
        |x, p| covariance_eci_to_vnc(*x, p, variant),
        x_eci,
        covariances,
    )
}
...
mod tests {
    ...
    fn test_batch_vnc_match_scalar() {
        ...
        let jac = jacobians_vnc_to_eci(&chiefs, variant);
        let jac_body = jacobians_vnc_to_inertial_for_body(&chiefs, gm, variant);
        let jac_inv = jacobians_eci_to_vnc(&chiefs, variant);
        let jac_inv_body = jacobians_inertial_to_vnc_for_body(&chiefs, gm, variant);
        let cov = covariances_vnc_to_eci(&chiefs, &covs, variant).unwrap();
        let cov_body =
            covariances_vnc_to_inertial_for_body(&chiefs, &covs[..1], gm, variant).unwrap();
        let cov_inv = covariances_eci_to_vnc(&chiefs[..1], &covs, variant).unwrap();
        let cov_inv_body =
            covariances_inertial_to_vnc_for_body(&chiefs, &covs, gm, variant).unwrap();
        ...
        for i in 0..3 {
            ...
            assert_eq!(jac[i], jacobian_vnc_to_eci(chiefs[i], variant));
            assert_eq!(
                jac_body[i],
                jacobian_vnc_to_inertial_for_body(chiefs[i], gm, variant)
            );
            assert_eq!(jac_inv[i], jacobian_eci_to_vnc(chiefs[i], variant));
            assert_eq!(
                jac_inv_body[i],
                jacobian_inertial_to_vnc_for_body(chiefs[i], gm, variant)
            );
            assert_eq!(cov[i], covariance_vnc_to_eci(chiefs[i], &covs[i], variant));
            assert_eq!(
                cov_body[i],
                covariance_vnc_to_inertial_for_body(chiefs[i], &covs[0], gm, variant)
            );
            assert_eq!(
                cov_inv[i],
                covariance_eci_to_vnc(chiefs[0], &covs[i], variant)
            );
            assert_eq!(
                cov_inv_body[i],
                covariance_inertial_to_vnc_for_body(chiefs[i], &covs[i], gm, variant)
            );
        ...
        assert!(covariances_vnc_to_eci(&chiefs[..2], &covs, variant).is_err());

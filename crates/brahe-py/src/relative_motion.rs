...
/// frame.
///
/// With `R` the SEZ-to-ECEF rotation and `omega` the SEZ angular velocity from `omega_sez`,
/// the Jacobian is `[[R, 0], [R @ skew(omega), R]]` for the rotating variant and
/// `[[R, 0], [0, R]]` for the inertial snapshot, which treats the SEZ axes as fixed relative to
/// ECEF (a fixed site's axes never rotate, so the two variants then agree).
///
/// Args:
///     x_ecef (numpy.ndarray or list): 6D state vector of the site in the ECEF frame [x, y, z, vx, vy, vz] (m, m/s), shape (6,), or a batch of
///         vectors with the 6 components along `axis` (for example shape (n, 6)).
///     variant (OrbitRelativeFrameVariant): Whether the SEZ axes rotate with the site or are frozen at the epoch
///     axis (int, optional): The axis along which the 6 components of a single vector
///         lie; the remaining axes enumerate the batch. For a batch of shape (n, 6)
///         the components lie along the last axis, so the default `-1` applies; a
///         (6, n) column layout uses `axis=0`.
///
/// Returns:
///     numpy.ndarray: 6x6 Jacobian such that `P_ecef = J @ P_sez @ J.T`, shape (6, 6), or the batch dimensions
///         followed by (6, 6) for batched input.
///
/// Example:
///     ```python
...
///     r_site = bh.position_geodetic_to_ecef(np.array([30.0, 45.0, 500.0]), bh.AngleFormat.DEGREES)
///     x_site = np.array([r_site[0], r_site[1], r_site[2], 0.0, 0.0, 0.0])
///     j = bh.jacobian_sez_to_ecef(x_site, bh.OrbitRelativeFrameVariant.ROTATING)
///     ```
#[pyfunction]
#[pyo3(signature = (x_ecef, variant, axis=-1))]
#[pyo3(text_signature = "(x_ecef, variant, axis=-1)")]
#[pyo3(name = "jacobian_sez_to_ecef")]
fn py_jacobian_sez_to_ecef<'py>(
    py: Python<'py>,
    x_ecef: &Bound<'py, PyAny>,
    variant: &PyOrbitRelativeFrameVariant,
    axis: isize,
    ...
    dispatch_vec_matrix::<6, 6>(
        ...
        x_ecef,
        axis,
        |x| relative_motion::jacobian_sez_to_ecef(x, variant),
        |xs| relative_motion::jacobians_sez_to_ecef(xs, variant),
    )
}
...
///
/// Returns:
///     numpy.ndarray: 6x6 Jacobian such that `P_sez = J @ P_ecef @ J.T`, shape (6, 6), or the batch dimensions
///         followed by (6, 6) for batched input.
///
/// Example:
///     ```python
...
///     r_site = bh.position_geodetic_to_ecef(np.array([30.0, 45.0, 500.0]), bh.AngleFormat.DEGREES)
///     x_site = np.array([r_site[0], r_site[1], r_site[2], 0.0, 0.0, 0.0])
///     j = bh.jacobian_ecef_to_sez(x_site, bh.OrbitRelativeFrameVariant.ROTATING)
///     ```
#[pyfunction]
#[pyo3(signature = (x_ecef, variant, axis=-1))]
#[pyo3(text_signature = "(x_ecef, variant, axis=-1)")]
#[pyo3(name = "jacobian_ecef_to_sez")]
fn py_jacobian_ecef_to_sez<'py>(
    py: Python<'py>,
    x_ecef: &Bound<'py, PyAny>,
    variant: &PyOrbitRelativeFrameVariant,
    axis: isize,
    ...
    dispatch_vec_matrix::<6, 6>(
        ...
        x_ecef,
        axis,
        |x| relative_motion::jacobian_ecef_to_sez(x, variant),
        |xs| relative_motion::jacobians_ecef_to_sez(xs, variant),
    )
}
...
/// frame.
///
/// Applies the congruence `P_ecef = J @ P_sez @ J.T` with `J` from `jacobian_sez_to_ecef`, and
...
///     x_ecef (numpy.ndarray or list): 6D state vector of the site in the ECEF frame [x, y, z, vx, vy, vz] (m, m/s), shape (6,), or a batch of
///         vectors with the 6 components along `axis` (for example shape (n, 6)).
///     covariance (numpy.ndarray): 6x6 state covariance in SEZ axes (m^2, m^2/s, m^2/s^2), shape (6, 6), or an (n, 6, 6) batch stacked along the leading axis; either argument may be single and broadcasts
///     variant (OrbitRelativeFrameVariant): Whether the SEZ axes rotate with the site or are frozen at the epoch
///     axis (int, optional): The axis along which the 6 components of a single vector
///         lie; the remaining axes enumerate the batch. For a batch of shape (n, 6)
///         the components lie along the last axis, so the default `-1` applies; a
...
///     r_site = bh.position_geodetic_to_ecef(np.array([30.0, 45.0, 500.0]), bh.AngleFormat.DEGREES)
///     x_site = np.array([r_site[0], r_site[1], r_site[2], 0.0, 0.0, 0.0])
///     p_ecef = bh.covariance_sez_to_ecef(
...
#[pyo3(name = "covariance_sez_to_ecef")]
fn py_covariance_sez_to_ecef<'py>(
    py: Python<'py>,
    x_ecef: &Bound<'py, PyAny>,
    covariance: &Bound<'py, PyAny>,
    variant: &PyOrbitRelativeFrameVariant,
    ...
    dispatch_vec_covariance::<6>(
        ...
        covariance,
        axis,
        |x, p| relative_motion::covariance_sez_to_ecef(x, p, variant),
        |xs, ps| relative_motion::covariances_sez_to_ecef(xs, ps, variant),
    )
}
...
/// SEZ axes.
///
/// Applies the congruence `P_sez = J @ P_ecef @ J.T` with `J` from `jacobian_ecef_to_sez`, and
/// symmetrizes the result. In the inertial-snapshot variant the SEZ axes are treated as fixed
/// relative to ECEF.
///
/// Args:
...
///
/// Returns:
///     numpy.ndarray: 6x6 state covariance in SEZ axes (m^2, m^2/s, m^2/s^2), shape (6, 6). For batched input, when `x_ecef` is batched the output is its batch dimensions followed by (6, 6), except a length-1 `x_ecef` batch broadcast against n covariances yields (n, 6, 6); when only `covariance` is batched the output is (n, 6, 6).
///
/// Example:
///     ```python
///     import brahe as bh
...
///     r_site = bh.position_geodetic_to_ecef(np.array([30.0, 45.0, 500.0]), bh.AngleFormat.DEGREES)
///     x_site = np.array([r_site[0], r_site[1], r_site[2], 0.0, 0.0, 0.0])
///     p_sez = bh.covariance_ecef_to_sez(
///         x_site, np.eye(6) * 100.0, bh.OrbitRelativeFrameVariant.ROTATING
///     )
///     ```
#[pyfunction]
#[pyo3(signature = (x_ecef, covariance, variant, axis=-1))]
#[pyo3(text_signature = "(x_ecef, covariance, variant, axis=-1)")]
#[pyo3(name = "covariance_ecef_to_sez")]
fn py_covariance_ecef_to_sez<'py>(
    py: Python<'py>,
    x_ecef: &Bound<'py, PyAny>,
    covariance: &Bound<'py, PyAny>,
    variant: &PyOrbitRelativeFrameVariant,
    ...
    dispatch_vec_covariance::<6>(
        ...
        covariance,
        axis,
        |x, p| relative_motion::covariance_ecef_to_sez(x, p, variant),
        |xs, ps| relative_motion::covariances_ecef_to_sez(xs, ps, variant),
    )
}

...
fn main() {
    ...
    let gravity_dynamics = |_t: f64, state: &DVector<f64>, _params: Option<&DVector<f64>>| -> Result<DVector<f64>, brahe::utils::BraheError> {
        ...
        Ok(DVector::from_iterator(6, v.iter().chain(a.iter()).copied()))
    };
    ...
    let j_forward = jacobian_forward.compute(t, &state, None).unwrap();
    let j_central = jacobian_central.compute(t, &state, None).unwrap();
    let j_backward = jacobian_backward.compute(t, &state, None).unwrap();

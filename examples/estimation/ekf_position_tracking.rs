...
fn main() {
    ...
    let ekf_prop = bh::propagators::DNumericalOrbitPropagator::new(
        epoch,
        initial_state,
        prop_config,
        bh::propagators::force_model_config::ForceModelConfig::two_body_gravity(),
        None, None, None, Some(p0),
    ).unwrap();
    ...
    let mut ekf = bh::estimation::ExtendedKalmanFilter::new(
        bh::estimation::DynamicsSource::OrbitPropagator(ekf_prop),
        models,
        bh::estimation::EKFConfig { process_noise: None, store_records: true },
    ).unwrap();

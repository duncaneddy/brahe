...
/// let result = rkf45.step(0.0, state, Some(0.5));
/// ```
    ...
    fn step_internal(
        ...
        loop {
            ...
            for i in 0..6 {
                ...
                if compute_phi || compute_sens {
                    let a_i = self.varmat.as_ref().expect("varmat required").compute(
                        t_i,
                        state_i.clone(),
                        params,
                    );
...
mod tests {
    ...
    fn test_rkf45s_integrator_parabola() {
        ...
        while t < 1.0 {
            ...
            let result = rkf45.step(t, state, Some(dt));
            state = result.state;
            t += result.dt_used;
        }
    ...
    fn test_rkf45s_integrator_orbit() {
        ...
        while epc < epcf {
            ...
            let result = rkf45.step(epc - epc0, state, Some(dt));
            state = result.state;
            epc += result.dt_used;
        }
    ...
    fn test_rkf45s_step_size_increases() {
        ...
        let result = rkf45.step(0.0, state, Some(dt_initial));
    ...
    fn test_rkf45s_config_parameters() {
        ...
        let state = SVector::<f64, 1>::new(0.0);
        let result = rkf45.step(0.0, state, Some(0.01));
    ...
    fn test_rkf45s_no_limits() {
        ...
        let state = SVector::<f64, 1>::new(0.0);
        let result = rkf45.step(0.0, state, Some(0.001));
    ...
    fn test_rkf45s_stm_accuracy() {
        ...
        let result =
            rkf45.step_with_varmat(0.0, state0, SMatrix::<f64, 6, 6>::identity(), Some(dt));
        ...
        for i in 0..6 {
            ...
            let result_pert = rkf45.step(0.0, state0_pert, Some(dt));
    ...
    fn test_rkf45s_stm_vs_direct_perturbation() {
        ...
        for step in 0..num_steps {
            // Propagate with STM
            let result = rkf45.step_with_varmat(t, state, phi, Some(dt));
            let state_new = result.state;
            let phi_new = result.phi;
            let dt_used = result.dt_used;

            // Propagate perturbed state directly
            let result_pert = rkf45.step(t, state_pert, Some(dt));
    ...
    fn test_rkf45d_stm_vs_direct_perturbation() {
        ...
        for step in 0..num_steps {
            ...
            let result = rkf45.step_with_varmat(t, state.clone(), phi.clone(), Some(dt));
            let state_new = result.state;
            let phi_new = result.phi.unwrap();
            let dt_used = result.dt_used;

            // Propagate perturbed state directly
            let result_pert = rkf45.step(t, state_pert.clone(), Some(dt));
    ...
    fn test_rkf45_s_vs_d_consistency() {
        ...
        let result_s = rkf45_s.step(0.0, state_s, Some(dt));
        let result_d = rkf45_d.step(0.0, state_d, Some(dt));
    ...
    fn test_rkf45s_backward_integration() {
        ...
        let mut t = 0.0;
        while t < 100.0 {
            let result = rkf45.step(t, state_fwd, Some(dt_forward));
        ...
        while t > 0.0 {
            ...
            let result = rkf45.step(t, state_back, Some(dt_back));
    ...
    fn test_rkf45d_varmat_sensmat() {
        ...
        let result_pert = rkf45_pert.step(0.0, state0_pert, Some(dt));
    ...
    fn test_rkf45s_varmat_sensmat() {
        ...
        while t_pert < 1.0 {
            ...
            let result = rkf45_pert.step(t_pert, state_pert, Some(dt));

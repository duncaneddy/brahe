...
def test_get_ut1_utc(iau2000_standard_filepath):
    ...
    eop = brahe.FileEOPProvider.from_standard_file(iau2000_standard_filepath, True, "Zero")
    brahe.set_global_eop_provider_from_file_provider(eop)
...
def test_get_dxdy(iau2000_standard_filepath):
    ...
    assert dX == pytest.approx(( 0.265*brahe.AS2RAD +  0.268*brahe.AS2RAD)/2.0 * 1.0e-3, abs=1e-12)
    assert dY == pytest.approx((-0.067*brahe.AS2RAD + -0.067*brahe.AS2RAD)/2.0 * 1.0e-3, abs=1e-12)

...
not by hand. `test-examples`, `test-example`, `make-plots`, and `make-plot`
depend on `download-resources`, which warms every network resource once,
serially, before their parallel pools run; on a machine without network
access, run them with `BRAHE_NETWORK_MODE=offline` against a previously
warmed `BRAHE_CACHE`. Examples flagged `NETWORK` fetch data that cannot be

...
def orbital_period(
    semi_major_axis: Annotated[
        float, typer.Argument(help="The semi-major axis of the orbit")
    ],
    gm: Annotated[
        float, typer.Option(help="The gravitational parameter of the central body")
    ] = None,
    units: Annotated[
    ...
    if gm is not None:
        typer.echo(
            f"{brahe.orbital_period_general(semi_major_axis, gm) / divisor:{format_string}}"
        )
    else:
...
@app.command()
def sma_from_period(
    period: Annotated[float, typer.Argument(help="The orbital period")],
    units: Annotated[
    ...
    if gm is not None:
        typer.echo(
            f"{brahe.semimajor_axis_from_orbital_period_general(period * multiplier, gm):{format_string}}"
        )
    else:
        typer.echo(
            f"{brahe.semimajor_axis_from_orbital_period(period * multiplier):{format_string}}"
        )
...
def anomaly_conversion(
    ...
    if input_anomaly == OrbitalAnomaly.mean:
        if output_anomaly == OrbitalAnomaly.eccentric:
            typer.echo(
                f"{brahe.anomaly_mean_to_eccentric(anomaly, eccentricty, as_degrees):{format_string}}"
            )
            return
        elif output_anomaly == OrbitalAnomaly.true:
            typer.echo(
                f"{brahe.anomaly_mean_to_true(anomaly, eccentricty, as_degrees):{format_string}}"
            )
            return
    elif input_anomaly == OrbitalAnomaly.eccentric:
        if output_anomaly == OrbitalAnomaly.mean:
            typer.echo(
                f"{brahe.anomaly_eccentric_to_mean(anomaly, eccentricty, as_degrees):{format_string}}"
            )
            return
        elif output_anomaly == OrbitalAnomaly.true:
            typer.echo(
                f"{brahe.anomaly_eccentric_to_true(anomaly, eccentricty, as_degrees):{format_string}}"
            )
            return
    elif input_anomaly == OrbitalAnomaly.true:
        if output_anomaly == OrbitalAnomaly.mean:
            typer.echo(
                f"{brahe.anomaly_true_to_mean(anomaly, eccentricty, as_degrees):{format_string}}"
            )
            return
        elif output_anomaly == OrbitalAnomaly.eccentric:
            typer.echo(
                f"{brahe.anomaly_true_to_eccentric(anomaly, eccentricty, as_degrees):{format_string}}"
            )
            return
...
def sun_sync_inclination(
    ...
    eccentricity: Annotated[
        float, typer.Argument(help="The eccentricity of the orbit")
    ],
    ...
    typer.echo(
        f"{brahe.sun_synchronous_inclination(semi_major_axis, eccentricity, as_degrees):{format_string}}"
    )
...
def perigee_velocity(
    ...
    typer.echo(
        f"{brahe.perigee_velocity(semi_major_axis, eccentricity):{format_string}}"
    )
...
def apogee_velocity(
    ...
    typer.echo(
        f"{brahe.apogee_velocity(semi_major_axis, eccentricity):{format_string}}"
    )

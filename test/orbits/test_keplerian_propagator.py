...
def create_cartesian_state():
    ...
    return state_osculating_to_cartesian(np.array([a, e, i, raan, argp, ma]), as_degrees=True)
...
def test_keplerianpropagator_new_invalid_angle_format():
    """Test that new() panics with invalid angle format"""
    epoch = Epoch.from_jd(TEST_EPOCH_JD, TimeSystem.UTC)
    elements = create_test_elements()
...
def test_keplerianpropagator_analyticpropagator_state_eci():
    ...
    computed_elements = state_cartesian_to_osculating(state, as_degrees=True)
...
def test_keplerianpropagator_analyticpropagator_state_ecef():
    ...
    computed_elements = state_cartesian_to_osculating(eci_state, as_degrees=True)
...
def test_keplerianpropagator_analyticpropagator_state_as_osculating_elements():
    ...
    osc_elements = propagator.state_as_osculating_elements(epoch + orbital_period(elements[0]), as_degrees=True)
    ...
    osc_elements_rad = propagator.state_as_osculating_elements(epoch + orbital_period(elements[0]), as_degrees=False)
    for i in range(2):
...
def test_keplerianpropagator_analyticpropagator_states_as_osculating_elements():
    ...
    traj = propagator.states_as_osculating_elements(epochs, as_degrees=True)
    assert len(traj) == 3
    ...
    traj_rad = propagator.states_as_osculating_elements(epochs, as_degrees=False)

...
def test_euler_angle_new():
    e1 = EulerAngle("XYZ", 30.0, 45.0, 60.0, True)
    assert e1.phi == 30.0 * DEG2RAD
    assert e1.theta == 45.0 * DEG2RAD
    assert e1.psi == 60.0 * DEG2RAD
    assert e1.order == "XYZ"

    e2 = EulerAngle("XYZ", pi/6.0, pi/4.0,  pi/3.0, False)
...
def test_euler_angle_to_quaternion():
    e = EulerAngle("XYZ", 0.0, 0.0, 0.0, True)
    q = e.to_quaternion()
    ...
    e = EulerAngle("XYZ", 30.0, 45.0, 60.0, True)
    q = e.to_quaternion()
...
def test_to_euler_angle_circular_xyx():
    e = EulerAngle("XYX", 30.0, 45.0, 60.0, True)

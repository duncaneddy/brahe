...
class TestNORADIDParsing:
    ...
    def test_parse_norad_id_invalid(self, id_str):
        ...
        with pytest.raises(
            RuntimeError
        ):  # brahe.BraheError in Rust becomes RuntimeError in Python
...
class TestNORADIDConversions:
    ...
    @pytest.mark.parametrize(
        "norad_id", [100000, 100001, 109999, 110000, 125678, 186789, 236789, 339999]
    )
    def test_norad_id_alpha5_numeric_round_trip(self, norad_id):
        ...
        assert norad_id == parsed_id, (
            f"Round trip failed for ID {norad_id}: {alpha5} -> {parsed_id}"
        )
...
class TestTLECircularity:
    ...
    def test_keplerian_tle_circularity(self):
        ...
        original_epoch = brahe.Epoch.from_datetime(
            2021, 1, 1, 12, 0, 0.0, 0.0, brahe.UTC
        )
        ...
        line1, line2 = brahe.keplerian_elements_to_tle(
            original_epoch, original_elements, norad_id
        )

        # Convert TLE back to Keplerian elements
        recovered_epoch, recovered_elements = brahe.keplerian_elements_from_tle(
            line1, line2
        )
        ...
        assert (
            abs(recovered_elements[0] - original_elements[0]) < 1.0
        )  # Semi-major axis (m)
        assert abs(recovered_elements[1] - original_elements[1]) < 1e-6  # Eccentricity
        assert (
            abs(recovered_elements[2] - original_elements[2]) < 1e-3
        )  # Inclination (degrees)
        assert (
            abs(recovered_elements[3] - original_elements[3]) < 1e-3
        )  # RAAN (degrees)
        assert (
            abs(recovered_elements[4] - original_elements[4]) < 1e-3
        )  # Argument of periapsis (degrees)
        assert (
            abs(recovered_elements[5] - original_elements[5]) < 1e-3
    ...
    def test_tle_keplerian_circularity(self):
        ...
        epoch, elements = brahe.keplerian_elements_from_tle(
            original_line1, original_line2
        )

        # Convert back to TLE
        recovered_line1, recovered_line2 = brahe.keplerian_elements_to_tle(
            epoch, elements, "25544"
        )
        ...
        _, original_elements = brahe.keplerian_elements_from_tle(
            original_line1, original_line2
        )
        _, recovered_elements = brahe.keplerian_elements_from_tle(
            recovered_line1, recovered_line2
        )
...
class TestEpochFromTLE:
    ...
    @pytest.mark.parametrize(
        "line1,year,month,day,hour,minute,second",
        [
            (
                "1 25544U 98067A   21001.50000000  .00001764  00000-0  40967-4 0  9997",
                2021,
    ...
    def test_epoch_from_tle_various_dates(
        self, line1, year, month, day, hour, minute, second
    ):
